  ...
  useEffect(() => {
    ...
    if (onSwipeUp) {
      onSwipeUp(() => {
        ...
        console.log('👆 Swipe up detected on HelenS');
      });
    }
    ...
    if (onSwipeDown) {
      onSwipeDown(() => {
        // For the portal concept, we don't want user controls
        // But keeping this for future flexibility  
        console.log('👇 Swipe down detected on HelenS');
      });
    }
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (maxDim > 0) {
          const scale = (isArMode ? 4 : 8) / maxDim;
        ...
        if (gltf.animations && gltf.animations.length > 0) {
          console.log('🎬 Found', gltf.animations.length, 'animation(s) in HelenS model');
          ...
          console.log('✅ HelenS animations initialized and auto-playing for portal effect');
        } else {
          console.log('ℹ️ No animations found in HelenS model');
    ...
    <>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div style={{
          ...
          <div style={{ color: 'yellow' }}>🖥️ HelenS PORTAL DEBUG</div>
