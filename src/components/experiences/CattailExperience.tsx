...
const CattailExperience: React.FC<CattailExperienceProps> = ({ 
  ...
  useEffect(() => {
    ...
    instructions.innerHTML = 'Explore the cattail marsh environment. Tap continue when ready.';
    ...
    loadingDiv.innerHTML = 'Loading Cattail Experience...';
    container.appendChild(loadingDiv);

    // Load the model
    const modelPath = getAssetPath('models/cattail.glb');
    console.log('🎯 Loading Cattail model:', modelPath);
      ...
      (gltf) => {
        ...
        if (isArMode && arPosition) {
          ...
          if (currentOverride) {
            ...
            console.log('🎯 Cattail positioned at TESTING override location:', model.position);
          } else {
            model.position.copy(arPosition);
            console.log('🎯 Cattail positioned at AR anchor location:', arPosition);
          }
        } else {
        ...
        console.log('✅ Cattail model loaded successfully');
      },
      (xhr) => {
        console.log(`Cattail model ${(xhr.loaded / xhr.total) * 100}% loaded`);
      },
      (error) => {
        console.error('❌ Error loading Cattail model:', error);
        if (container.contains(loadingDiv)) {
          loadingDiv.innerHTML = 'Error loading Cattail Experience<br>Model file may be missing';
        }
      }
    );
    ...
    <>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div style={{
          ...
          <div style={{ color: 'yellow' }}>🌾 CATTAIL DEBUG</div>
